# Documentação do MVP SaaS

Organização:

```text
docs/
├── arquitetura/   decisões estruturais e estratégia de branches
├── fases/         implementação e testes de cada fase do MVP
└── planejamento/  escopo, roadmap e decisões de produto
```

A documentação operacional do n8n pertence à branch `n8n-operacional`.

Fases recentes:

- [Fase 5 — Coleta histórica e dashboard](fases/fase-5-historico-dashboard.md)
- [Fase 6 — Automação da sincronização](fases/fase-6-automacao-sincronizacao.md)
- [Fase 7 — Observabilidade e falhas](fases/fase-7-observabilidade.md)
- [Fase 8 — Segurança da API](fases/fase-8-seguranca-api.md)
