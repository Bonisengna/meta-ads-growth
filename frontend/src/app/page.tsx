...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {view === "settings" ? <section className="workspace"><SettingsPanel token={session.access_token} clients={clients} /></section> : <section className="workspace" id="dashboard-top"><header><div><p className="eyebrow coral">{pageCopy.eyebrow}</p><h1>{pageCopy.title}</h1><p>{pageCopy.subtitle}</p></div><div className="profile"><span>{session.user.email?.slice(0, 1).toUpperCase()}</span><div><strong>{session.user.email?.split("@")[0]}</strong><small>Acesso protegido</small></div></div></header>
