    ...
    <div className="settings-layout"><nav className="settings-nav">
      ...
      {section === "clients" && <div className="client-manager">
        <div className="client-list-card">
          <div className="client-list-head"><div className="client-list-copy"><p className="eyebrow coral">CARTEIRA</p><h2>Clientes</h2><p className="client-list-intro">Selecione uma empresa para completar ou revisar o cadastro.</p></div><div className="client-picker-actions"><label className="client-picker"><span>Selecionar cliente</span><select aria-label="Selecionar cliente" value={clientForm.id} disabled={managedClients.length === 0} onChange={(event) => { const selected = managedClients.find((item) => item.id === event.target.value); if (selected) setClientForm(formFromClient(selected)); }}>{managedClients.length === 0 ? <option value="">Nada aqui</option> : <><option value="">Selecione um cliente</option>{managedClients.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.status === "ACTIVE" ? "Ativo" : "Arquivado"}</option>)}</>}</select></label>{data?.system_admin && <button type="button" onClick={() => setClientForm({ ...emptyClientForm })}>Novo cliente</button>}</div></div>
        </div>
