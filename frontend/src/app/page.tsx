...
import { isActiveWithoutDelivery, operationNameMatches, searchCampaignOperations } from "@/lib/operations";
...
const chartGuides: Record<string, IndicatorGuide> = {
  ...
  "Estados e regiões": { title: "Desempenho por estado ou região", meaning: "Mostra quais localidades concentram entrega e resultados.", direction: "Regiões com CPA menor e conversões recorrentes merecem atenção.", attention: "Não confunda poucos resultados baratos com tendência: valide volume e qualidade comercial." },
...
function metricValue(key: keyof Metrics, value: number | null | undefined) {
  ...
  if (["ctr", "link_ctr", "landing_page_view_rate", "landing_page_conversion_rate", "hook_rate", "thruplay_rate", "video_p25_rate", "video_p50_rate", "video_p75_rate", "video_p95_rate"].includes(key)) return `${decimal.format(value)}%`;
...
function InvestmentPacingPanel({ dashboard }: { dashboard: Dashboard }) {
  const pacing = dashboard.investment_pacing;
  const labels = { NOT_CONFIGURED: "Orçamento não configurado", BELOW: "Ritmo abaixo do planejado", ON_TRACK: "Ritmo dentro do planejado", ABOVE: "Ritmo acima do planejado" } as const;
  if (pacing.monthly_budget == null) return <section className="pacing-panel not-configured"><div><p className="eyebrow coral">RITMO DE INVESTIMENTO</p><h2>Defina o orçamento mensal do cliente</h2><p>Com esse valor, o painel calcula saldo, consumo e projeção até o fim do mês.</p></div><span>{labels.NOT_CONFIGURED}</span></section>;
  const consumed = Math.max(0, pacing.percent_consumed ?? 0);
  return <section className={`pacing-panel ${pacing.pace_status.toLowerCase()}`} aria-labelledby="pacing-title"><div className="pacing-head"><div><p className="eyebrow coral">RITMO DE INVESTIMENTO</p><h2 id="pacing-title">Orçamento do mês</h2><p>Dia {pacing.days_elapsed} de {pacing.days_in_month} · {decimal.format(pacing.elapsed_percent)}% do mês transcorrido</p></div><span>{labels[pacing.pace_status]}</span></div><div className="pacing-values"><div><small>Orçamento</small><strong>{currency.format(pacing.monthly_budget)}</strong></div><div><small>Investido</small><strong>{currency.format(pacing.spent)}</strong></div><div><small>Restante</small><strong>{currency.format(pacing.remaining ?? 0)}</strong></div><div><small>Projeção</small><strong>{pacing.projected_spend == null ? "—" : currency.format(pacing.projected_spend)}</strong></div></div><div className="pacing-track" aria-label={`${decimal.format(consumed)}% do orçamento consumido`}><i style={{ width: `${Math.min(consumed, 100)}%` }} /></div><div className="pacing-foot"><span>{decimal.format(consumed)}% consumido</span><span>Esperado até hoje: {currency.format(pacing.expected_spend_to_date ?? 0)}</span><span>Projeção: {pacing.projected_percent == null ? "—" : `${decimal.format(pacing.projected_percent)}%`}</span></div></section>;
}

type OperationColumn = "spend" | "results" | "cpl" | "link_ctr" | "frequency" | "cpm" | "budget";
const operationColumnLabels: Record<OperationColumn, string> = { spend: "Investimento", results: "Resultados", cpl: "CPL", link_ctr: "CTR de link", frequency: "Frequência média", cpm: "CPM", budget: "Orçamento" };

function CreativeSummary({ ad }: { ad: AdOperation }) {
  const image = ad.thumbnail_url ?? ad.image_url;
  return <div className="creative-summary">{image ? <span className="creative-image" role="img" aria-label={`Criativo do anúncio ${ad.name}`} style={{ backgroundImage: `url(${image})` }} /> : <span className="creative-fallback" aria-hidden="true">{ad.creative_type === "VIDEO" ? "▶" : "▧"}</span>}<div><strong>{ad.name}</strong><small>{ad.creative_type ?? "Formato não informado"}{ad.video_duration_seconds ? ` · ${decimal.format(ad.video_duration_seconds)}s` : ""}</small>{ad.headline && <b>{ad.headline}</b>}{ad.primary_text && <p>{ad.primary_text}</p>}{ad.call_to_action_type && <em>CTA: {ad.call_to_action_type.replaceAll("_", " ")}</em>}</div></div>;
}

function OperationalCampaignTable({ items }: { items: CampaignOperation[] }) {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState<"ALL" | CampaignOperation["status"]>("ALL");
  ...
  const [columns, setColumns] = useState<OperationColumn[]>(["spend", "results", "cpl", "link_ctr", "frequency", "cpm", "budget"]);
  const [expandedCampaigns, setExpandedCampaigns] = useState<string[]>([]);
  const [expandedAdsets, setExpandedAdsets] = useState<string[]>([]);
  const [compared, setCompared] = useState<string[]>([]);
  useEffect(() => { const frame = window.requestAnimationFrame(() => { const saved = window.localStorage.getItem("descompliads.campaign.columns"); if (saved) { try { const parsed = JSON.parse(saved) as OperationColumn[]; if (parsed.length) setColumns(parsed); } catch { return; } } }); return () => window.cancelAnimationFrame(frame); }, []);
  const metricNumber = (item: CampaignOperation, key: OperationColumn) => key === "results" ? item.metrics.leads + item.metrics.conversations : key === "budget" ? item.configured_budget ?? -1 : item.metrics[key] ?? -1;
  const visible = useMemo(() => searchCampaignOperations(items, search)
    .filter((item) => status === "ALL" || item.status === status)
    .filter((item) => !onlyActiveWithoutDelivery || isActiveWithoutDelivery(item))
    .sort((a, b) => (metricNumber(a, sort) - metricNumber(b, sort)) * (descending ? -1 : 1)), [descending, items, onlyActiveWithoutDelivery, search, sort, status]);
  const toggleExpanded = (id: string, level: "campaign" | "adset") => { const setter = level === "campaign" ? setExpandedCampaigns : setExpandedAdsets; setter((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]); };
  const toggleColumn = (column: OperationColumn) => setColumns((current) => { const next = current.includes(column) ? current.filter((item) => item !== column) : [...current, column]; const safe: OperationColumn[] = next.length ? next : ["spend"]; window.localStorage.setItem("descompliads.campaign.columns", JSON.stringify(safe)); return safe; });
  const formatCell = (metrics: Metrics, column: OperationColumn, budget?: number | null, utilization?: number | null) => { if (column === "spend") return currency.format(metrics.spend); if (column === "results") return integer.format(metrics.leads + metrics.conversations); if (column === "cpl") return metrics.cpl == null ? "—" : currency.format(metrics.cpl); if (column === "link_ctr") return metrics.link_ctr == null ? "—" : `${decimal.format(metrics.link_ctr)}%`; if (column === "frequency") return metrics.frequency == null ? "—" : decimal.format(metrics.frequency); if (column === "cpm") return metrics.cpm == null ? "—" : currency.format(metrics.cpm); return budget == null ? "Não configurado" : `${currency.format(budget)} · ${utilization == null ? "—" : `${decimal.format(utilization)}%`}`; };
  const comparedItems = compared.map((id) => items.find((item) => item.id === id)).filter(Boolean) as CampaignOperation[];
  return <section className="panel operation-panel"><div className="panel-title"><div><p className="eyebrow">OPERAÇÃO</p><h2>Campanhas, conjuntos e anúncios</h2></div><span>{visible.length} campanhas</span></div><div className="operation-toolbar"><label>Pesquisar<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Campanha, conjunto ou anúncio" /></label><label>Situação da campanha<select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="ALL">Todas</option><option value="ACTIVE">Ativas</option><option value="PAUSED">Pausadas</option><option value="ARCHIVED">Arquivadas</option></select></label><label>Ordenar por<select value={sort} onChange={(event) => setSort(event.target.value as OperationColumn)}>{Object.entries(operationColumnLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><button className="sort-direction" onClick={() => setDescending((current) => !current)}>{descending ? "Maior primeiro ↓" : "Menor primeiro ↑"}</button><button className={`delivery-filter${onlyActiveWithoutDelivery ? " active" : ""}`} aria-pressed={onlyActiveWithoutDelivery} onClick={() => setOnlyActiveWithoutDelivery((current) => !current)}>Ativas sem entrega</button><details className="column-picker"><summary>Escolher colunas</summary><div>{Object.entries(operationColumnLabels).map(([key, label]) => <label key={key}><input type="checkbox" checked={columns.includes(key as OperationColumn)} onChange={() => toggleColumn(key as OperationColumn)} />{label}</label>)}</div></details></div>{onlyActiveWithoutDelivery && <p className="delivery-rule">Sem entrega = campanha ativa com investimento zero e zero impressões no período selecionado.</p>}{comparedItems.length > 0 && <div className="campaign-comparison"><div><strong>Comparação lado a lado</strong><span>{comparedItems.length === 1 ? "Selecione mais uma campanha" : "Compare eficiência e entrega"}</span></div>{comparedItems.map((item) => <article key={item.id}><b>{item.name}</b><span>{currency.format(item.metrics.spend)} investidos</span><span>{item.metrics.leads + item.metrics.conversations} resultados</span><span>CTR {item.metrics.link_ctr == null ? "—" : `${decimal.format(item.metrics.link_ctr)}%`}</span></article>)}<button onClick={() => setCompared([])}>Limpar</button></div>}<div className="table-wrap operation-table-wrap"><table className="operation-table"><thead><tr><th>Estrutura</th><th>Comparar</th><th>Situação</th>{columns.map((column) => <th key={column}>{operationColumnLabels[column]}</th>)}</tr></thead><tbody>{visible.length === 0 ? <tr><td colSpan={columns.length + 3} className="empty-cell">Nenhuma campanha, conjunto ou anúncio corresponde aos filtros.</td></tr> : visible.map((campaign) => { const campaignAutoExpanded = !!search.trim() && !operationNameMatches(campaign.name, search); const campaignExpanded = expandedCampaigns.includes(campaign.id) || campaignAutoExpanded; return <Fragment key={campaign.id}><tr className={isActiveWithoutDelivery(campaign) ? "no-delivery" : ""}><td><button className="expand-button" aria-label={campaignAutoExpanded ? `Campanha ${campaign.name} expandida pela pesquisa` : `${campaignExpanded ? "Recolher" : "Expandir"} campanha ${campaign.name}`} aria-expanded={campaignExpanded} aria-disabled={campaignAutoExpanded} disabled={campaignAutoExpanded} onClick={() => toggleExpanded(campaign.id, "campaign")}>{campaignExpanded ? "−" : "+"}</button><strong>{campaign.name}</strong>{isActiveWithoutDelivery(campaign) && <span className="delivery-badge">Sem entrega</span>}</td><td><input type="checkbox" aria-label={`Comparar ${campaign.name}`} checked={compared.includes(campaign.id)} onChange={() => setCompared((current) => current.includes(campaign.id) ? current.filter((id) => id !== campaign.id) : current.length < 2 ? [...current, campaign.id] : [current[1], campaign.id])} /></td><td><span className={`status ${campaign.status.toLowerCase()}`}>{campaign.status === "ACTIVE" ? "Ativa" : campaign.status === "PAUSED" ? "Pausada" : "Arquivada"}</span></td>{columns.map((column) => <td key={column}>{formatCell(campaign.metrics, column, campaign.configured_budget, campaign.budget_utilization)}</td>)}</tr>{campaignExpanded && campaign.adsets.map((adset) => { const adsetAutoExpanded = !!search.trim() && !operationNameMatches(adset.name, search) && adset.ads.some((ad) => operationNameMatches(ad.name, search)); const adsetExpanded = expandedAdsets.includes(adset.id) || adsetAutoExpanded; return <Fragment key={adset.id}><tr className="adset-row"><td><span className="hierarchy-line" /><button className="expand-button" aria-label={adsetAutoExpanded ? `Conjunto ${adset.name} expandido pela pesquisa` : `${adsetExpanded ? "Recolher" : "Expandir"} conjunto ${adset.name}`} aria-expanded={adsetExpanded} aria-disabled={adsetAutoExpanded} disabled={adsetAutoExpanded} onClick={() => toggleExpanded(adset.id, "adset")}>{adsetExpanded ? "−" : "+"}</button><div><strong>{adset.name}</strong><small>Conjunto · {adset.optimization_goal ?? "objetivo não informado"}</small></div></td><td /><td><span className={`status ${adset.status.toLowerCase()}`}>{adset.status === "ACTIVE" ? "Ativo" : adset.status === "PAUSED" ? "Pausado" : "Arquivado"}</span></td>{columns.map((column) => <td key={column}>{formatCell(adset.metrics, column, adset.configured_budget, adset.budget_utilization)}</td>)}</tr>{adsetExpanded && adset.ads.map((ad) => <tr className="ad-row" key={ad.id}><td><CreativeSummary ad={ad} /></td><td /><td><span className={`status ${ad.status.toLowerCase()}`}>{ad.status === "ACTIVE" ? "Ativo" : ad.status === "PAUSED" ? "Pausado" : "Arquivado"}</span></td>{columns.map((column) => <td key={column}>{formatCell(ad.metrics, column)}</td>)}</tr>)}</Fragment>; })}</Fragment>; })}</tbody></table></div></section>;
}
...
function BreakdownChart({ title, subtitle, points, note }: { title: string; subtitle: string; points: BreakdownPoint[]; note?: string }) {
  const [showGuide, setShowGuide] = useState(false);
...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [clientPage, accountPage, campaignPage, dashboardData, improvementPage, healthResult] = await Promise.all([
        ...
        apiGet<Dashboard>(`/api/v1/dashboard?${query({ days: filters.days, client_id: filters.clientId, meta_account_id: filters.accountId, campaign_id: filters.campaignId, include_exact: 1 })}`, token),
        apiGet<Page<Improvement>>("/api/v1/improvements?page=1&page_size=20", token),
        apiGetStatus<MetaHealth>("/api/v1/health/meta", token).catch(() => null),
      ]);
  ...
  const systemStatus = metaHealth?.status === "HEALTHY"
    ? { label: "Dados atualizados", tone: "healthy" }
    : metaHealth?.status === "DEGRADED"
      ? { label: "Coleta parcial", tone: "degraded" }
      : metaHealth?.status === "UNHEALTHY"
        ? { label: "Coleta com falha", tone: "unhealthy" }
        : metaHealth?.status === "UNCONFIGURED"
          ? { label: "Meta não configurada", tone: "unhealthy" }
        : { label: "Verificando coleta", tone: "unknown" };
    ...
    {view === "settings" ? <section className="workspace"><SettingsPanel token={session.access_token} clients={clients} userEmail={session.user.email ?? "Usuário autenticado"} onUpdatePassword={updateAuthenticatedPassword} onClientsChanged={setClients} onSessionExpired={expireSession} /></section> : <section className={`workspace${view === "dashboard" ? " dashboard-workspace" : ""}`} id="dashboard-top"><header className={view === "dashboard" ? "dashboard-header" : ""}><div><p className="eyebrow coral">{pageCopy.eyebrow}</p><h1>{view === "dashboard" ? <>Olá, <span>{userName}</span></> : pageCopy.title}</h1><p>{pageCopy.subtitle}</p>{view === "dashboard" && <div className="context-summary" aria-label="Contexto selecionado"><span>{selectedClientName}</span><i aria-hidden="true">›</i><span>{selectedAccountName}</span><i aria-hidden="true">›</i><span>{selectedCampaignName}</span></div>}</div></header>
      ...
      {dashboard && <>{dashboard.metrics.spend === 0 && <div className="guidance-banner"><div><strong>Nenhum investimento encontrado neste período.</strong><span>{metaHealth && metaHealth.status !== "HEALTHY" ? "A coleta precisa ser concluída antes de confirmar que não houve entrega." : "Escolha um período em que as campanhas estiveram ativas ou mantenha esta visão para acompanhar a próxima coleta."}</span></div><button onClick={() => updateFilters({ ...filters, days: 360 })}>Ver 360 dias</button></div>}<section className="period-line"><span>{friendlyDate(dashboard.period.date_from)} → {friendlyDate(dashboard.period.date_to)}</span><small>comparado a {friendlyDate(dashboard.previous_period.date_from)} → {friendlyDate(dashboard.previous_period.date_to)}</small></section>
        ...
        {view === "campaigns" && <><section className="metrics-grid campaign-metrics"><MetricCard label="Investimento" metricKey="spend" metrics={dashboard.metrics} change={dashboard.change_percent.spend} /><MetricCard label="Impressões" metricKey="impressions" metrics={dashboard.metrics} change={dashboard.change_percent.impressions} /><MetricCard label="Alcance" metricKey="reach" metrics={dashboard.metrics} change={dashboard.change_percent.reach} /><MetricCard label="Cliques no link" metricKey="link_clicks" metrics={dashboard.metrics} change={dashboard.change_percent.link_clicks} /><MetricCard label="Frequência" metricKey="frequency" metrics={dashboard.metrics} change={dashboard.change_percent.frequency} /><MetricCard label="CPM" metricKey="cpm" metrics={dashboard.metrics} change={dashboard.change_percent.cpm} /></section>
          ...
          <OperationalCampaignTable items={dashboard.campaign_operations} /></>}
        {view === "analyses" && <><section className="funnel-section"><div className="funnel-heading"><span>1</span><div><p className="eyebrow">TOPO DO FUNIL</p><h2>O criativo prende atenção?</h2></div></div><div className="funnel-grid"><FunnelMetric label="Hook Rate" value={metricValue("hook_rate", dashboard.metrics.hook_rate)} source={`${integer.format(dashboard.metrics.video_views_3s)} visualizações de 3s`} description="Visualizações de 3 segundos ÷ impressões. Abaixo de 25–30% pode indicar um gancho fraco." /><FunnelMetric label="ThruPlay Rate" value={metricValue("thruplay_rate", dashboard.metrics.thruplay_rate)} source={`${integer.format(dashboard.metrics.thruplays)} ThruPlays`} description="ThruPlays ÷ reproduções iniciadas. Mostra se o roteiro mantém a atenção." /><FunnelMetric label="CTR de link" value={metricValue("link_ctr", dashboard.metrics.link_ctr)} description="Mede se a mensagem e a oferta geram interesse em quem viu o anúncio." /><FunnelMetric label="CPM" value={metricValue("cpm", dashboard.metrics.cpm)} description="Termômetro de competição, qualidade da audiência e possível fadiga." /><FunnelMetric label="Frequência no período" value={metricValue("frequency", dashboard.metrics.frequency)} warning={(dashboard.metrics.frequency ?? 0) >= 3} description="Acima de 3–4 pode sinalizar saturação em públicos frios pequenos." /></div><div className="retention-bars">{[["25%",dashboard.metrics.video_p25_rate],["50%",dashboard.metrics.video_p50_rate],["75%",dashboard.metrics.video_p75_rate],["95%",dashboard.metrics.video_p95_rate]].map(([label,value]) => <div key={label as string}><span>{label}</span><i><b style={{width:`${Math.min(Number(value ?? 0),100)}%`}} /></i><strong>{value == null ? "—" : `${decimal.format(Number(value))}%`}</strong></div>)}</div></section>
          <section className="funnel-section"><div className="funnel-heading"><span>2</span><div><p className="eyebrow">MEIO DO FUNIL</p><h2>A página e a oferta convertem?</h2></div></div><div className="funnel-grid"><FunnelMetric label="Visualizações da página" value={integer.format(dashboard.metrics.landing_page_views)} description="Quantidade de pessoas que efetivamente carregaram a página de destino." /><FunnelMetric label="Taxa de chegada à página" value={metricValue("landing_page_view_rate", dashboard.metrics.landing_page_view_rate)} description="LPV ÷ cliques no link. Quedas apontam lentidão ou instabilidade da página." /><FunnelMetric label="Custo por visita" value={metricValue("cost_per_landing_page_view", dashboard.metrics.cost_per_landing_page_view)} description="Investimento ÷ LPV. Mostra quanto custa levar uma pessoa até a página." /><FunnelMetric label="Conversão da página" value={metricValue("landing_page_conversion_rate", dashboard.metrics.landing_page_conversion_rate)} description="Leads ÷ LPV. Isola a capacidade da página e da oferta de gerar cadastros." /></div></section>
