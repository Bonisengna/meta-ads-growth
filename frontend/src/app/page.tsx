...
import type { AdOperation, BreakdownPoint, Campaign, CampaignOperation, Client, Dashboard, EntityPerformance, Improvement, ManagedClient, MetaAccount, Metrics, Page, Recommendation, SettingsData } from "@/lib/types";
...
function OperationalCampaignTable({ items }: { items: CampaignOperation[] }) {
  ...
  const visible = useMemo(() => [...items].filter((item) => (status === "ALL" || item.status === status) && item.name.toLocaleLowerCase("pt-BR").includes(search.toLocaleLowerCase("pt-BR"))).sort((a, b) => (metricNumber(a, sort) - metricNumber(b, sort)) * (descending ? -1 : 1)), [descending, items, search, sort, status]);
  ...
  return <section className="panel operation-panel"><div className="panel-title"><div><p className="eyebrow">OPERAÇÃO</p><h2>Campanhas, conjuntos e anúncios</h2></div><span>{visible.length} campanhas</span></div><div className="operation-toolbar"><label>Pesquisar<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nome da campanha" /></label><label>Situação<select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="ALL">Todas</option><option value="ACTIVE">Ativas</option><option value="PAUSED">Pausadas</option><option value="ARCHIVED">Arquivadas</option></select></label><label>Ordenar por<select value={sort} onChange={(event) => setSort(event.target.value as OperationColumn)}>{Object.entries(operationColumnLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><button className="sort-direction" onClick={() => setDescending((current) => !current)}>{descending ? "Maior primeiro ↓" : "Menor primeiro ↑"}</button><details className="column-picker"><summary>Escolher colunas</summary><div>{Object.entries(operationColumnLabels).map(([key, label]) => <label key={key}><input type="checkbox" checked={columns.includes(key as OperationColumn)} onChange={() => toggleColumn(key as OperationColumn)} />{label}</label>)}</div></details></div>{comparedItems.length > 0 && <div className="campaign-comparison"><div><strong>Comparação lado a lado</strong><span>{comparedItems.length === 1 ? "Selecione mais uma campanha" : "Compare eficiência e entrega"}</span></div>{comparedItems.map((item) => <article key={item.id}><b>{item.name}</b><span>{currency.format(item.metrics.spend)} investidos</span><span>{item.metrics.leads + item.metrics.conversations} resultados</span><span>CTR {item.metrics.link_ctr == null ? "—" : `${decimal.format(item.metrics.link_ctr)}%`}</span></article>)}<button onClick={() => setCompared([])}>Limpar</button></div>}<div className="table-wrap operation-table-wrap"><table className="operation-table"><thead><tr><th>Estrutura</th><th>Comparar</th><th>Situação</th>{columns.map((column) => <th key={column}>{operationColumnLabels[column]}</th>)}</tr></thead><tbody>{visible.length === 0 ? <tr><td colSpan={columns.length + 3} className="empty-cell">Nenhuma campanha corresponde aos filtros.</td></tr> : visible.map((campaign) => <Fragment key={campaign.id}><tr className={campaign.has_delivery ? "" : "no-delivery"}><td><button className="expand-button" aria-expanded={expandedCampaigns.includes(campaign.id)} onClick={() => toggleExpanded(campaign.id, "campaign")}>{expandedCampaigns.includes(campaign.id) ? "−" : "+"}</button><strong>{campaign.name}</strong>{!campaign.has_delivery && <span className="delivery-badge">Sem entrega</span>}</td><td><input type="checkbox" aria-label={`Comparar ${campaign.name}`} checked={compared.includes(campaign.id)} onChange={() => setCompared((current) => current.includes(campaign.id) ? current.filter((id) => id !== campaign.id) : current.length < 2 ? [...current, campaign.id] : [current[1], campaign.id])} /></td><td><span className={`status ${campaign.status.toLowerCase()}`}>{campaign.status === "ACTIVE" ? "Ativa" : campaign.status === "PAUSED" ? "Pausada" : "Arquivada"}</span></td>{columns.map((column) => <td key={column}>{formatCell(campaign.metrics, column, campaign.configured_budget, campaign.budget_utilization)}</td>)}</tr>{expandedCampaigns.includes(campaign.id) && campaign.adsets.map((adset) => <Fragment key={adset.id}><tr className="adset-row"><td><span className="hierarchy-line" /><button className="expand-button" aria-expanded={expandedAdsets.includes(adset.id)} onClick={() => toggleExpanded(adset.id, "adset")}>{expandedAdsets.includes(adset.id) ? "−" : "+"}</button><div><strong>{adset.name}</strong><small>Conjunto · {adset.optimization_goal ?? "objetivo não informado"}</small></div></td><td /><td><span className={`status ${adset.status.toLowerCase()}`}>{adset.status === "ACTIVE" ? "Ativo" : adset.status === "PAUSED" ? "Pausado" : "Arquivado"}</span></td>{columns.map((column) => <td key={column}>{formatCell(adset.metrics, column, adset.configured_budget, adset.budget_utilization)}</td>)}</tr>{expandedAdsets.includes(adset.id) && adset.ads.map((ad) => <tr className="ad-row" key={ad.id}><td><CreativeSummary ad={ad} /></td><td /><td><span className={`status ${ad.status.toLowerCase()}`}>{ad.status === "ACTIVE" ? "Ativo" : ad.status === "PAUSED" ? "Pausado" : "Arquivado"}</span></td>{columns.map((column) => <td key={column}>{formatCell(ad.metrics, column)}</td>)}</tr>)}</Fragment>)}</Fragment>)}</tbody></table></div></section>;
}
    ...
    <div className="settings-layout"><nav className="settings-nav">
      ...
      {section === "clients" && <div className="client-manager">
        <div className="client-list-card">
          <div className="client-list-head"><div className="client-list-copy"><p className="eyebrow coral">CARTEIRA</p><h2>Clientes</h2><p className="client-list-intro">Cadastre uma empresa para organizar contas, campanhas e análises.</p></div>{data?.system_admin && <button type="button" onClick={() => setClientForm({ ...emptyClientForm })}>Novo cliente</button>}</div>
        </div>
